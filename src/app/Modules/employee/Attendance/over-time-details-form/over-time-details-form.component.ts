...
export class OverTimeDetailsFormComponent implements OnInit {
  ...
  constructor(
    public DigiofficecorehrService: DigiofficecorehrService,
    public router: Router,
    public Datepipe: DatePipe,
    private activatedroute: ActivatedRoute,
    public dialogRef: MatDialogRef<OverTimeDetailsDashComponent>,
  ...
  public submit() {
    this.DigiofficecorehrService.ProjectAttachments(
      this.attachments21
    ).subscribe({
      next: (data) => {
        debugger;
  ...
  public InsertStaffOverTimeDetails() {
    ...
    if (
      ...
      var eb = {
        ...
        Attachment:
          this.attachmentsurl[0] == '' ? null : this.attachmentsurl[0],
      ...
      this.DigiofficecorehrService.InsertStaffOverTimeDetails(eb).subscribe({
        next: (data) => {
          debugger;
          if (data == 0) {
            this.loader = false;
  ...
  public uploadmultipleimages(id: any) {
    ...
    for (let i = 0; i < this.attachmentsurl.length; i++) {
      ...
      this.DigiofficecorehrService.InsertStaffOverTimeDetailsAttachment(
        entity
      ).subscribe((data) => {
        this.loader = false;
      });
  ...
  public update() {
    ...
    this.DigiofficecorehrService.UpdateStaffShiftDetails(entity).subscribe(
      (res) => {
        debugger;
  ...
  getOverTimeDate() {
    debugger;
    this.showPopup = 0;
    this.DigiofficecorehrService.GetApprovedStaffLeavesByStaffID(
    ...
    this.DigiofficecorehrService.GetAttendanceByEmployeeID(
      localStorage.getItem('staffid'),
      this.date,
      this.date
    ).subscribe({
      next: (data) => {
        ...
        if (temp.length > 0) {
          if (temp[0].undertime == 'Yes') {
            this.loader = false;
            Swal.fire(
              'You are Undertime on this date , So you can not Apply OT.'
            );
            this.loader = false;
          }
  ...
  public checkStartDate() {
    ...
    this.updatedStartTime =
      this.add_leading0(this.startTime.getHours()) +
      ':' +
      this.add_leading0(this.startTime.getMinutes());
    ...
    this.DigiofficecorehrService.GetHolidaybit(
      this.date,
      this.staffID
    ).subscribe((data1) => {
      ...
      if (temp1?.length > 0) {
        ...
        this.DigiofficecorehrService.GetStaffShiftDetailsByStaffID(
          localStorage.getItem('staffid')
        ).subscribe({
          next: (data) => {
            debugger;
            let temp = data.filter(
              (x) =>
                x.filterenddate >= this.date &&
                x.filterdate <= this.date &&
                x.approve == 1
            );
            if (temp.length == 0) {
              this.loader = false;
  ...
  public duration() {
    ...
    let dayname = date.toLocaleDateString('en-US', { weekday: 'long' });
    this.updatedEndTime =
      this.add_leading0(this.endTime.getHours()) +
      ':' +
      this.add_leading0(this.endTime.getMinutes());
    if (
      (this.endTime - this.startTime) / 60000 < 60 &&
                ...
                this.DigiofficecorehrService.GetAttendanceByEmployeeID(
                  ...
                  next: (data) => {
                    ...
                    let temp: any = data.filter(
                      (x) =>
                        x.filterdate == this.date &&
                        x.userID == localStorage.getItem('staffid')
                    );
                    var newdate = new Date();
                    const theDate = new Date(this.date);
                    var day = 60 * 60 * 24 * 1000;
                    var endDate = new Date(theDate.getTime() + day);
                    var tomotrwdate = this.Datepipe.transform(
                      endDate,
                      'yyyy-MM-dd'
                    );
                    let temp1: any = data.filter(
                      (x) =>
                        x.filterdate == tomotrwdate &&
                        x.userID == localStorage.getItem('staffid')
                    );
                    if (temp.length == 0) {
                      this.loader = false;
                    ...
                    if (
                      temp[0].webSignoutDate == null ||
                      temp[0].webSignoutDate == undefined
                    ) {
                      Swal.fire(
                        'You can not Apply Overtime as you are not Punched Out on this day'
                      );
                      this.loader = false;
                      this.showPopup = 1;
                      this.messageId = 20;
                    ...
                    if (this.showDetails != false) {
                      this.DigiofficecorehrService.GetOtNightOt(
                        this.updatedStartTime,
                        this.updatedEndTime,
                        this.shift,
                        this.staffID,
                        ...
                        next: (data) => {
                          ...
                          let temp: any = data;
                          (this.ot =
                            temp[0].normalOT == null
                              ? 0
                              : temp[0].normalOT + ' Hours'),
                            (this.nightOT =
                              temp[0].nightOt == null
                                ? 0
                                : temp[0].nightOt + ' Hours'),
                            (this.noOfHours =
                              temp[0].normalOT == null ? 0 : temp[0].normalOT),
                            (this.nightOTHours =
                              temp[0].nightOt == null ? 0 : temp[0].nightOt),
                            (this.exccessNormalOT =
                              temp[0].exccess8NormalOt == null
                                ? 0
                                : temp[0].exccess8NormalOt),
                            (this.exccessNightOT =
                              temp[0].exccess8NightOt == null
                                ? 0
                                : temp[0].exccess8NightOt),
                            (this.nSDRegular =
                              temp[0].nsD_REGULAR == null
                                ? 0
                                : temp[0].nsD_REGULAR),
                            (this.restNightOT =
                              temp[0].restNightOt == null
                                ? 0
                                : temp[0].restNightOt),
                            (this.restNormalOT =
                              temp[0].restNormalOT == null
                                ? 0
                                : temp[0].restNormalOT),
                            (this.exccessRestNormalOT =
                              temp[0].exccessRestNormalOt == null
                                ? 0
                                : temp[0].exccessRestNormalOt),
                            (this.restExccessNightOT =
                              temp[0].restExccessNightOt == null
                                ? 0
                                : temp[0].restExccessNightOt),
                            (this.legalNightOT =
                              temp[0].legalNightOt == null
                                ? 0
                                : temp[0].legalNightOt),
                            (this.legalNormalOT =
                              temp[0].legalNormalOT == null
                                ? 0
                                : temp[0].legalNormalOT),
                            (this.legalExccessNormalOT =
                              temp[0].legalExccessNormalOt == null
                                ? 0
                                : temp[0].legalExccessNormalOt),
                            (this.legalExccessNightOT =
                              temp[0].legalExccessNightOt == null
                                ? 0
                                : temp[0].legalExccessNightOt),
                            (this.specialHoliday =
                              temp[0].specialHoliday == null
                                ? 0
                                : temp[0].specialHoliday),
                            (this.specialNightOT =
                              temp[0].specialNightOt == null
                                ? 0
                                : temp[0].specialNightOt),
                            (this.specialNormalOT =
                              temp[0].specialNormalOT == null
                                ? 0
                                : temp[0].specialNormalOT),
                            (this.specialExccessNormalOT =
                              temp[0].specialExccessNormalOt == null
                                ? 0
                                : temp[0].specialExccessNormalOt),
                            (this.specialExccessNightOT =
                              temp[0].specialExccessNightOt == null
                                ? 0
                                : temp[0].specialExccessNightOt),
                            (this.specialRestNightOT =
                              temp[0].specialRestNightOt == null
                                ? 0
                                : temp[0].specialRestNightOt),
                            (this.specialRestNormalOT =
                              temp[0].specialRestNormalOT == null
                                ? 0
                                : temp[0].specialRestNormalOT),
                            (this.specialRestExccessNormalOT =
                              temp[0].specialRestExccessNormalOt == null
                                ? 0
                                : temp[0].specialRestExccessNormalOt),
                            (this.specialRestExccessNightOT =
                              temp[0].specialRestExccessNightOt == null
                                ? 0
                                : temp[0].specialRestExccessNightOt),
                            (this.legalRestNightOT =
                              temp[0].legalRestNightOt == null
                                ? 0
                                : temp[0].legalRestNightOt),
                            (this.legalRestNormalOT =
                              temp[0].legalRestNormalOT == null
                                ? 0
                                : temp[0].legalRestNormalOT),
                            (this.legalExccessRestNormalOT =
                              temp[0].legalExccessRestNormalOt == null
                                ? 0
                                : temp[0].legalExccessRestNormalOt),
                            (this.legalExccessRestNightOT =
                              temp[0].legalExccessRestNightOt == null
                                ? 0
                                : temp[0].legalExccessRestNightOt);
                          this.loader = false;
                          this.showDetails = true;
                          Swal.fire(
                            'Please Check All OT Details Before Submitting'
                          );
                        },
                      });
                      this.loader = false;
                ...
                this.DigiofficecorehrService.GetAttendanceByEmployeeID(
                  ...
                  next: (data) => {
                    ...
                    if (temp5[0].restdays.includes(dayname) == true) {
                      ...
                      if (
                        temp[0].minutesdiff < 480 &&
                        temp[0].minutesdiff > 0
                      ) {
                        this.showPopup = 1;
                        this.messageId = 23;
                        this.showbtn = false;
                    ...
                    if (this.showDetails != false) {
                      this.DigiofficecorehrService.GetOtNightOt(
                        ...
                        next: (data) => {
                          ...
                          console.log(temp);
                          (this.ot =
                            parseFloat(temp[0].normalOT) +
                            parseFloat(temp[0].restNormalOT) +
                            parseFloat(temp[0].legalNormalOT) +
                            parseFloat(temp[0].specialNormalOT) +
                            parseFloat(temp[0].specialRestNormalOT) +
                            parseFloat(temp[0].legalRestNormalOT) +
                            ' Hours'),
                            (this.nightOT =
                              parseFloat(temp[0].nightOt) +
                              parseFloat(temp[0].restNightOt) +
                              parseFloat(temp[0].legalNightOt) +
                              parseFloat(temp[0].specialNightOt) +
                              parseFloat(temp[0].specialRestNightOt) +
                              parseFloat(temp[0].legalRestNightOt) +
                              ' Hours'),
                            (this.noOfHours =
                              temp[0].normalOT == null ? 0 : temp[0].normalOT),
                            (this.nightOTHours =
