...
export class NewLeaveRequestComponent implements OnInit {
  ...
  public Save() {
    ...
    else if (this.LeaveReason == undefined || this.SDateOfLeave == undefined || this.EDateOfLeave == undefined
      || this.CoveringStaff == undefined || this.CoveringStaff == "" || this.LeaveReason == "" ||
      this.SDateOfLeave == null || this.EDateOfLeave == null || this.SDateOfLeave == "" ||
      this.EDateOfLeave == "" || this.LeaveType == "" ||
      this.LeaveType == undefined || this.LeaveReason == null || this.CoveringStaff == null ||
      this.LeaveType == null) {
              ...
              this.DigiofficeService.GetMyDetailsByStaffID(localStorage.getItem('staffid'))
                .subscribe({
                  next: data => {
                    debugger
                    let temp: any = data;
                    if (temp[0].gender == 'Female' && temp[0].maternitityLeaveEntitlement - temp[0].maternitityLeaveBalance > 0) {
                      var obj: any = {};
                      obj["id"] = this.LeaveTypeList[i].id;
                      obj["short"] = this.LeaveTypeList[i].short;
                      this.newLeaveTypeList.push(obj);
              ...
              this.DigiofficeService.GetMyDetailsByStaffID(localStorage.getItem('staffid'))
                .subscribe({
                  next: data => {
                    debugger
                    let temp: any = data;
                    if (temp[0].gender == 'Male' && temp[0].paternitityLeaveEntitlement - temp[0].paternitityLeaveBalance > 0) {
                      var obj: any = {};
                      obj["id"] = this.LeaveTypeList[i].id;
                      obj["short"] = this.LeaveTypeList[i].short;
                      this.newLeaveTypeList.push(obj);
      ...
      this.DigiofficeService.GetMyDetails()
        .subscribe({
          next: data => {
            ...
            if (temp[0].maternitityLeaveEntitlement - temp[0].maternitityLeaveBalance <= 0) {
      ...
      this.DigiofficeService.GetMyDetails()
        .subscribe({
          next: data => {
            ...
            if (temp[0].paternitityLeaveEntitlement - temp[0].paternitityLeaveBalance <= 0) {
              ...
              .subscribe({
                next: data => {
                  ...
                  if (this.LeaveType == 10041) {
                    ...
                    this.totalleavesilldate = temp[0].service_Incentive_LeaveBalance;
                  }
                  else if (this.LeaveType == 10044) {
                  ...
                  if (this.totalleavesilldate >= yearlyLimit) {
                    Swal.fire('Yearly Quota Completd for this Leave Type. Please Apply LWOP');
                    this.loader = false;
                    this.hidesavebtn = true;
                    // this.showPopup = 1;
                    // this.messageId = 100;
                  }
                  var totalvailbeltilltoday = totaleaveentiletment - this.totalleavesilldate;
                  var totalvailbeltilltoday = totalvailbeltilltoday < 0 ? 0 : totalvailbeltilltoday;

                  if (totalvailbeltilltoday < diffDays) {
                    Swal.fire('Applied Days are more than Available Leave Days. Please Apply LWOP');
                    this.loader = false;
                    this.hidesavebtn = true;
                  }else{
                    ...
                    Swal.fire(({
                      ...
                      type: 'info',
                      html:
                        '<p style="font-size: 24px;text-align: start;margin-left: 135px;"> Yearly Limit   : ' + yearlyLimit +
                        '       <br>' +
                        'Used Till Date: ' + this.totalleavesilldate +
                        '       <br>' +
                        'Available Till Date: ' + totalvailbeltilltoday +
                        '       <br>' +
                        'Applied Days: ' + diffDays +
                        '       <br>' +
                        'LWOP Day: ' + this.lopdays +
                        '       <br>' +
                        '</p>'
                      ,
                      ...
                      if (result.value == true) {
                        ...
                        this.DigiofficeService.GetStaffLeaves(localStorage.getItem('staffid'), 1, '01-02-2022', '01-02-2099')
                          .subscribe({
                            next: data => {
                              debugger
                              let temp: any = data.filter(x => x.staffID == localStorage.getItem('staffid') && (x.status == 'Manager Approved' || x.status == 'Manager Pending'));
                              for (let i = 0; i < temp.length; i++) {
                                if (temp[i].filterdate <= this.SDateOfLeave && temp[i].filterdate1 >= this.SDateOfLeave) {
                                  this.leaveaready = 1;
                                } else {
                                  this.leaveaready = 0;
                                }
                              ...
                              if (this.leaveaready == 1) {
                                ...
                                this.DigiofficeService.InsertStaffLeavesWeb(eb)
                                  .subscribe({
                                    next: data => {
                                      debugger
  ...
  public sendemail() {
    ...
    var entity1 = {
      ...
      'emailbody': 'Hi  <br> Your Employee ' + this.UserName + ' has Applied leave in Digi-Office., <br> Please Login in DigiOffice to Approve. <br><br>' + '<br>  <br> Thanks <br> Team Digi-Office',
  ...
  public Getenddate() {
    ...
    this.DigiofficeService.GetHolidaybit(this.SDateOfLeave, this.staffid).subscribe(data1 => {
