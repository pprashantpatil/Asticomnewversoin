...
export class AddressDetailsWizardComponent implements OnInit {
  ...
  public GetStaffAllowanceDetails() {
    debugger
    this.loader = true;
    this.DigiofficeService.GetStaffAllowanceDetailsByStaffID(this.StaffID)
      .subscribe({
        next: data => {
          debugger
  ...
  public DeleteStaffAllowanceDetails(ID: any) {
    ...
    Swal.fire({
      ...
      if (result.value == true) {
        this.DigiofficeService.DeleteStaffAllowanceDetails(ID)
          .subscribe({
            next: data => {
              debugger
              this.loader = false;
  ...
  public UpdateStaffAllowanceDetails() {
    ...
    this.DigiofficeService.UpdateStaffAllowanceDetails(entity)
      .subscribe({
        next: data => {
          debugger
          Swal.fire("Updated Successfully");
  ...
  public getAllowanceID(id: any) {
    ...
    this.DigiofficeService.GetStaffAllowanceDetailsByStaffID(this.StaffID).subscribe(
      data => {
        ...
        if ((this.datepipe.transform(this.allowanceList[0].startdate, 'yyyy-MM-dd')) == "1990-01-01") {
          this.startdate = " "
        }
        else {
          this.startdate = this.datepipe.transform(this.allowanceList[0].startdate, 'yyyy-MM-dd')
        }
        if ((this.datepipe.transform(this.allowanceList[0].enddate, 'yyyy-MM-dd')) == "1990-01-01") {
          this.endate = " "
        }
        else {
          this.endate = this.datepipe.transform(this.allowanceList[0].enddate, 'yyyy-MM-dd')
        }
        this.loader = false;
      });
  }
