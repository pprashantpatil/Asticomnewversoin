...
@Component({
  ...
  styleUrls: ['./generate-preliminary-report.component.css']
})
export class GeneratePreliminaryReportComponent implements OnInit {
  ...
  public GetPayrollFilesDetails() {
    this.DigiofficeService.GetPayrollFilesDetails()
      .subscribe({
        next: data => {
          //debugger
          this.FilesDetails = data.filter(x => x.finalrun == 0);
          this.uniquelist = [];
          this.loader = false;
        }, error: (err) => {
          // Swal.fire('Issue in Getting Position');
  ...
  public createPayrollFileentry() {
    ...
    this.DigiofficeService.InsertPayrollfiles(obj)
      .subscribe({
        next: data => {
          debugger
          if (data != 0) {
  ...
  public createNewPayrollEntry() {
    ...
    this.DigiofficeService.InsertPayrollfileswithoutdeatecheck(obj)
      .subscribe({
        next: data => {
          debugger
          this.payrollid = data;
  ...
  public Generatezip() {
    ...
    //         let blob: any = content;
    //         let qwddwde = 'DGinput'
    //         // fs.saveAs(blob, qwddwde);
    //         var file = new File([blob], qwddwde + '.zip');
    //         let body = new FormData();
    //         body.append('Dan', file);
  ...
  exporttoexcel2() {
    ...
    this.DigiofficeService.CreateExcelFiles(obj1)
      .subscribe({
        next: data => {
          //debugger 
          debugger
